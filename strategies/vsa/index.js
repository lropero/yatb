...
class Strategy {
  static analyze (candles) {
    return new Promise((resolve, reject) => {
      ...
      if (candles[0].close > candles[1].high && candles[1].close > candles[2].high) { // Price moving up
      ...
      } else if (candles[0].close < candles[1].low && candles[1].close < candles[2].low) { // Price moving down
        if (candles[0].volume > candles[1].volume && candles[0].volume > candles[2].volume) { // Volume larger than previous 2 volumes
