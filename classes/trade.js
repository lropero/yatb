...
class Trade {
  constructor (advisorId, buy, chartId, funds, info, isLong, log, order, sell, show, strategy, stream, updateFunds, who) {
    ...
    this.log({ level: this.isLong ? 'long' : 'short', message: `${chalk.underline('T' + this.id)}: ${this.info.symbol} ${this.quantity}${chalk.cyan('@')}${this.price.toFixed(decimalPlaces)} (TRGT ${this.targetPrice.toFixed(decimalPlaces)}) (STOP ${this.stopPrice.toFixed(decimalPlaces)}) ${chalk.black(this.who)}` })
  ...
  toString () {
    ...
    const { tickSize } = this.info.filters.find((filter) => filter.filterType === 'PRICE_FILTER')
    const decimalPlaces = tickSize.replace(/0+$/, '').split('.')[1].length + 1
    return getIcon() + ' ' + chalk[this.isOpen ? 'white' : 'gray'](`${chalk.underline('T' + this.id)}: ${this.info.symbol} ${this.quantity}${chalk.cyan('@')}${this.price.toFixed(decimalPlaces)} (TRGT ${this.targetPrice.toFixed(decimalPlaces)}) (STOP ${this.stopPrice.toFixed(decimalPlaces)}) ${chalk.gray(this.who)}`)
  }
}
